...
class HomeAssistantAdapter(BaseIntegration):
    """
    Async adapter that connects IoT-Claw to Home Assistant via the WebSocket API.

    Implements BaseIntegration (integration_id = "ha").
    Designed to be started via the IntegrationRegistry from main.py lifespan.
    """
    ...
    async def start(self) -> None:
        """Launch the WebSocket reconnection loop as a background asyncio.Task."""
        asyncio.create_task(self.run())
        print(f"[HA] HomeAssistantAdapter started — targeting {self._host}:{self._port}")

    async def stop(self) -> None:
        """Signal the run loop to exit gracefully."""
        self._running = False
        print("[HA] HomeAssistantAdapter stopping")

    # ── BaseIntegration command dispatch ──────────────────────────────────────

    async def send_command(self, device: dict, command: str, params: dict) -> dict:
        """
        Route a command to Home Assistant via call_service().

        ``command`` becomes the ``state`` key (ON / OFF / TOGGLE).
        Any additional protocol-specific params (brightness_pct, color_temp_kelvin,
        rgb_color, temperature, hvac_mode, media_content_id) are passed through
        as service_data.
        """
        entity_id = device["name"]
        data: dict = {"state": command.upper()}
        # Merge in any extra params, excluding the redundant "command" key
        for k, v in params.items():
            if k != "command" and v is not None:
                data[k] = v
        result = await self.call_service(entity_id, data=data)
    ...
    async def _ws_broadcast(self, message: dict) -> None:
        """Safely broadcast to all WebSocket clients, swallowing errors."""
        try:
    ...
    async def call_service(
        ...
        If service is None, auto-derives it from domain + action in data.
        """
    ...
    async def run(self):
        ...
        print(f"[HA] Adapter loop starting — targeting {self._host}:{self._port}")
        while self._running:
            try:
                ...
                await self._ws_broadcast({"type": "ha_status", "connected": False})
                    ...
                    async for msg in ws:
                        if msg.type == aiohttp.WSMsgType.TEXT:
                            ...
                            if msg_type == "auth_required":
                                ...
                                await self._ws_broadcast({"type": "ha_status", "connected": True})
                                await self._fetch_and_register_states()
    ...
    async def _fetch_and_register_states(self):
        ...
        await self._ws_broadcast({"type": "state", "data": self.storage.get_all_devices()})
